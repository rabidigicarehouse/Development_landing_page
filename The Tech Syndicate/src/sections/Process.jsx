          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3 lg:grid-cols-5">
            {steps.map((step, idx) => (
              <motion.div
                ...
                className="group relative z-10 flex flex-col items-center overflow-hidden rounded-[3rem] border border-slate-200 bg-white p-8 text-center shadow-xl transition-all duration-500 hover:border-primary/50 dark:border-white/5 dark:bg-dark-card/40"
              >
                ...
                <div className="mb-8 flex h-20 w-20 items-center justify-center rounded-3xl border border-slate-200 bg-slate-50 text-slate-400 shadow-sm transition-all duration-700 group-hover:rotate-12 group-hover:scale-110 group-hover:text-primary dark:border-white/10 dark:bg-white/5">
                  {React.cloneElement(step.icon, { className: 'h-8 w-8' })}
                </div>

                <h4 className="font-heading mb-4 text-2xl font-black uppercase leading-none tracking-tighter text-slate-900 dark:text-white">{step.title}</h4>
                <p className="text-sm font-light leading-relaxed tracking-tight text-slate-600 dark:text-gray-400">{step.desc}</p>
